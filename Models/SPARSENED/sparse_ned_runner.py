...
from Models.SPARSENED.sparse_ned_biome_ae import BiomeAE, BiomeCCA, BiomeAESnip, BiomeLasso, BiomeMultiTaskLasso, BiomeLinear, BiomeOneLayerSnip, \
    BiomeLassoAESnip

from Models.SPARSENED.sparse_ned_utils import draw_weight_graph, write_matrix_to_csv, get_subgraph, consistency_index, prune_subgraph
...
def main(args):
    ...
    for fold in range(num_folds):
        ...
        for val_ind, (x1_val, x2_val, y_val, meaning) in enumerate(zip(x1_vals, x2_vals, y_vals, vals_meanings)):
            ...
            test_corrs1_cv[val_ind] = np.nan_to_num(
                [np.corrcoef(x2_val_hat[:, i], x2_val[:, i])[0, 1] for i in range(x2_val.shape[1])])
            ...
            if args.model == "CCA":
                ...
                latent_corrs_val_cv[val_ind] = [np.corrcoef(x1_c_val[:, i], x2_c_val[:, i])[0, 1] for i in
                                                range(x1_c_val.shape[1])]
            else:
            ...
            if args.draw_graph and args.sparse is not None and args.fea1 != "bac_fea":
                ...
                draw_weight_graph(prunedn12, prunedw12, graph_fname12,
                                  Name1, Name2,
                                  content.get("%s_DA" % args.fea1.replace("_fea", ""), None),
                                  content.get("%s_DA" % args.fea2.replace("_fea", ""), None), content["z_DA"])
    ...
    if args.model == "CCA":
        ...
        for v, meaning in enumerate(vals_meanings):
            plt.plot(np.arange(args.latent_size) + 1, latent_corrs[v], marker='.', c=colors[v],
                     label="val on %s" % (meaning))
    ...
    for v, meaning in enumerate(vals_meanings):  # for each choice of val selection, we take the mean of the folds
        ...
        if args.draw_graph and args.sparse is not None and args.fea1 != "bac_fea":
            ...
            for LINK_THRESHOLD in [0.0, 0.025, 0.05, 0.075, 0.1]:
                ...
                draw_weight_graph(subnodes, subweights, graph_fname, Name1, Name2, content["bac_group_DA"],
                                  content["met_group_DA"], content["z_DA"])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_root", type=str, default='./try_data')
