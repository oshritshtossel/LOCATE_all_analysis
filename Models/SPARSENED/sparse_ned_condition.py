...
if __name__ == '__main__':
    melon_pan = True
    for name in ["BGU", "He","Parkinson"]: #"BGU", "He","Jacob","Parkinson","Kim"
        metab_test = pd.read_csv(f"sparse_ned_corrs/{name}/test_real_values.csv",index_col=0)#1 for jacov amd kim
        metab_pred = pd.read_csv(f"sparse_ned_corrs/{name}/test_predictions.csv",index_col=0)#1 for jacov amd kim
        if name == "BGU":
            if melon_pan:
            ...
            if melon_pan:
                melon = pd.read_csv("melon_pan_data/Kim/metab.csv", index_col=0)
                metab_test = metab_test[melon.columns]
                metab_pred = metab_pred[melon.columns]
        ...
        plt.title(name)
        plt.legend()
        plt.savefig(f"sparse_ned_corrs/condition_sparsened/melon_metab/{name}.png")
        plt.show()
